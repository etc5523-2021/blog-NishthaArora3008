...
**Map showing that Mexico is one of the countries with highest mortality rate**
*(Please use plotly i.e. click on the country regions to see more information.)*

```{r map, echo=FALSE, fig.cap="Map showing countries with most deaths"}

map <- covid_data %>% 
  select(location, new_deaths) %>% na.omit() %>% 
  group_by(location) %>% 
...
fig <- plot_ly(map, type='choropleth', locations= map$iso3 , z= map$total_deaths, text= ~paste(
                      "<br><b>Country:</b> ", location,
                      "<br><b>HDI rank 2019:</b>", total_deaths), colorscale="distiller")
fig <- fig %>% colorbar(title = "Total deaths" )
fig <- fig %>% layout(
    title = "Map 1: Countries with highest mortality rate")
fig
```
...
- From Figure 1, it is seen that Mexico falls under the countries that have had high amount of deaths since 2020 till present (2021).
...
We can say that, Mexico had had high no. of deaths since 2020 and also the positive cases have gradually increased for both the years, but also the recovery rate for COVID-19 tested individuals has increased too approximately at the same pace. This justifies that the **Mexico health care has managed to control the situation and mortality**.
